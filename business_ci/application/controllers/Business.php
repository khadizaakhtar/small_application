...
class Business extends CI_Controller {
    ...
    public function company_verification() {
        ...
        $update = $this->business_model->update_company_verification($company_id, $status);
...
//print_r($company);die();
            ...
            $check_verify = $this->business_model->check_mobile_number($company['mobile_number']);

            if ($check_verify == 1) {

                $this->business_model->update_company_status($lastid);
                $this->session->set_flashdata("success", "Your company has been listed successfully!!!");
                ...
                //echo $code; exit;
                $message = $code;
                $res = $this->sendMessage($recipient,$message,$lastid);
                //echo $res; exit;

                $this->session->set_flashdata("success", "Your company will be listed after verification!!!");
    ...
    public function resend_verification_code() {
        ...
        $company_id = $this->uri->segment(2);
        $info = $this->option->get_by_id('company_info', 'company_id', $company_id); //$table, $fieldname, $id
       
       if($info->count_resend_code <=1) {   
            
        $count_resend_code = $info->count_resend_code + 1;    
        $newCode = str_pad(mt_rand(1, 999999), 6, '0', STR_PAD_LEFT);
        $update = $this->business_model->update_newcode($newCode,$company_id,$count_resend_code);
        ...
        //echo $code; exit;
        $message = $code;
        $this->sendMessage($recipient, $message, $company_id);
        $json = array();
        $json['count'] = $count_resend_code;
        $json['success'] = 1;
        }else{
        $json['failed'] = 0;
        }
    ...
    function sendMessage($recipient, $message,$company_id) {
    ...
    public function update_verfication() {
        ...
        $verification_info = $this->option->get_by_id('company_info', 'company_id', $company_id);  //echo $code.' '.$verification_info->code; exit;
        if ($code == $verification_info->code) { 
            ...
            redirect('listbusiness');
               ...
               $this->session->set_flashdata('error', 'Your have inserted wrong verification code!!!');
               redirect('verify_company/'.$company_id);
    ...
    public function check_mobilenumber_ajax() {

        $json = array();
        $mobile_no = $this->input->post('mobile_no');
        $check_mobilenumber = $this->business_model->get_number($mobile_no);
        //echo $check_mobilenumber; exit;
        if ($check_mobilenumber == 1) {
            $json['ok'] = 'ok';
        } else {
              $query = $this->db->query("
                UPDATE `company_info` SET `status` = 2 WHERE `mobile_number` ='" . $mobile_no . "' AND `status` = 0
               ");
            $json['ok'] = 'banned';
        }//print_r($json); die();
